...
import { FaFileExcel, FaFileCsv, FaTimes, FaUser, FaEnvelope, FaPhone, FaHome, FaStickyNote, FaCreditCard, FaMoneyBill, FaTruck, FaCalendarAlt, FaClipboardList } from "react-icons/fa";
...
const statusBadge = (status) => {
  ...
  switch (status) {
    case "pending": return <span className={`${base} badge-error badge-outline`}>pending</span>;
    case "partial": return <span className={`${base} badge-info badge-outline`}>partial</span>;
    case "paid": return <span className={`${base} badge-success`}>paid</span>;
    case "order-request-received": return <span className={`${base} badge-secondary`}>order request received</span>;
    case "consultation-in-progress": return <span className={`${base} badge-primary`}>consultation in progress</span>;
    case "order-canceled": return <span className={`${base} badge-error`}>order canceled</span>;
    case "awaiting-advance-payment": return <span className={`${base} badge-warning`}>awaiting advance payment</span>;
    case "advance-payment-received": return <span className={`${base} badge-info`}>advance payment received</span>;
    case "design-in-progress": return <span className={`${base} badge-accent`}>design in progress</span>;
    case "awaiting-design-approval": return <span className={`${base} badge-warning`}>awaiting design approval</span>;
    case "production-started": return <span className={`${base} badge-info`}>production started</span>;
    case "production-in-progress": return <span className={`${base} badge-info`}>production in progress</span>;
    case "ready-for-delivery": return <span className={`${base} badge-success`}>ready for delivery</span>;
    case "out-for-delivery": return <span className={`${base} badge-primary`}>out for delivery</span>;
    case "order-completed": return <span className={`${base} badge-success`}>order completed</span>;
...
export default function Order() {
  ...
  const staffMap = useMemo(() => {
    const map = {};
    staffList.forEach(staff => { map[staff.staffId] = staff.name });
    return map;
  }, [staffList]);
  ...
  const fetchOrders = async () => {
    try {
      ...
      const data = await res.json();
      let sortedOrders = data.data.orders.map(order => ({
        ...order,
      ...
      sortedOrders.sort((a, b) => {
        ...
        if (sortField === "orderTotalPrice") return sortOrder === "asc" ? aVal - bVal : bVal - aVal;
        if (sortField === "deliveryDate") return sortOrder === "asc" ? new Date(aVal) - new Date(bVal) : new Date(bVal) - new Date(aVal);
  ...
  }, [search, sortField, sortOrder, staffList]);
    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex gap-2 items-center">
          <select className="select select-sm select-bordered" value={sortField} onChange={e => setSortField(e.target.value)}>
          ...
          <select className="select select-sm select-bordered" value={sortOrder} onChange={e => setSortOrder(e.target.value)}>
            <option value="desc">Descending</option>
            <option value="asc">Ascending</option>
          </select>
          <button onClick={exportToExcel} className="btn btn-success btn-sm text-white"><FaFileExcel className="mr-2" /> Excel</button>
          <button onClick={exportToCSV} className="btn btn-info btn-sm text-white"><FaFileCsv className="mr-2" /> CSV</button>
        </div>
      </div>
            ...
            <tbody>
              {currentOrders.map((order, i) => (
                <tr key={order.orderId || i} className="hover">
                  ...
                  <td>
                    <button onClick={() => setSelectedOrder(order)} className="btn btn-outline btn-xs bg-blue-700 text-white">
                      View
                    </button>
                  </td>
                  <td><InvoiceDownloadButton order={order} staffName={order.staffName} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <dialog className="modal modal-open">
          <div className="modal-box w-11/12 max-w-4xl">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              ...
              <button onClick={closeModal} className="btn btn-sm btn-circle btn-error text-white"><FaTimes /></button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
              <div className="space-y-1">
                ...
                <p className="flex items-center gap-2 "><FaUser /> <strong>Staff Name:</strong> {selectedOrder.staffName}</p>
              </div>
              <div className="space-y-1">
